...
// Creates Stripe PaymentIntents. The server requires a real Stripe secret;
// production code must never report a simulated payment as successful.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (items.length > 0) {
      params.set("description", `Mobicare Order: ${items.length} item(s)`);
    }
    ...
    if (!stripeResponse.ok) {
      ...
      return res.status(502).json({
        error: stripeData?.error?.message || "Unable to create payment.",
      });
    }
