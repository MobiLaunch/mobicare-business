...
export default function ProductDetail() {
  ...
  if (!product || !product.active) {
    return <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 py-12 text-center"><PageMeta description="The requested product was not found." title="Product Not Found | Mobicare" /><span aria-hidden="true" className="mb-5 flex size-20 items-center justify-center rounded-full bg-surface-secondary text-accent"><PackageX className="size-10" /></span><h2 className="m-0 mb-2 text-heading-xl font-extrabold">Product Not Found</h2><p className="m-0 mb-6 max-w-[400px] text-base text-muted">The item you are looking for may have been removed or is unavailable.</p><Button variant="primary" onPress={() => navigate("/shop")}><span>Back to Shop</span><ArrowRight aria-hidden="true" className="size-4" /></Button></div>;
  }
  ...
  return (
    <main className="mx-auto max-w-[1400px] overflow-x-hidden px-[clamp(12px,3vw,24px)] py-4 pb-16 sm:py-6">
      ...
      <Button aria-label="Go back" className="mb-4 inline-flex min-h-11 items-center gap-1.5 rounded-full px-3 py-2 font-semibold text-foreground transition-colors hover:bg-surface-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent sm:mb-5" variant="ghost" onPress={() => navigate(-1)}><ArrowLeft aria-hidden="true" className="size-4" /><span>Back</span></Button>
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2 lg:gap-10">
        ...
        <div className="flex min-w-0 flex-col lg:pt-2">
          ...
          {product.stock > 0 ? <div className="mb-3 flex w-full flex-col gap-3 min-[420px]:flex-row min-[420px]:items-center"><div aria-label="Quantity" className="flex h-[52px] w-full items-center justify-between rounded-full border border-border bg-surface-secondary px-2 min-[420px]:w-auto min-[420px]:justify-start"><button aria-label="Decrease quantity" className="flex size-11 items-center justify-center rounded-full text-foreground transition-colors hover:bg-surface focus-visible:outline-2 focus-visible:outline-accent disabled:opacity-40" disabled={qty <= 1} type="button" onClick={() => setQty((q) => Math.max(1, q - 1))}><Minus aria-hidden="true" className="size-4" /></button><span aria-live="polite" className="w-10 text-center text-base font-bold text-foreground">{qty}</span><button aria-label="Increase quantity" className="flex size-11 items-center justify-center rounded-full text-foreground transition-colors hover:bg-surface focus-visible:outline-2 focus-visible:outline-accent disabled:opacity-40" disabled={qty >= product.stock} type="button" onClick={() => setQty((q) => Math.min(product.stock, q + 1))}><Plus aria-hidden="true" className="size-4" /></button></div><Button className="h-[52px] w-full min-w-0 sm:min-w-[220px]" variant="primary" onPress={handleAddToCart}><ShoppingCart aria-hidden="true" className="size-[18px]" /><span className="truncate">Add to Cart — ${(product.price * qty).toFixed(2)}</span></Button></div> : <div className="mb-4 rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-center font-bold text-rose-600 dark:text-rose-400">Out of Stock — Check Back Soon</div>}
          {product.stock > 0 && product.stock < 5 && <p className="mb-4 flex items-center gap-1.5 text-label font-bold text-amber-600 dark:text-amber-400"><TriangleAlert aria-hidden="true" className="size-[18px]" />Only {product.stock} left in stock — order soon</p>}
          <article className="mb-5 mt-4 overflow-hidden rounded-[22px] border border-border bg-surface-secondary shadow-[0_4px_16px_rgba(0,0,0,0.04)] sm:mt-6 sm:rounded-[24px]">{trustRows.map((row, i) => <div key={row.title} className={`flex items-center gap-3.5 p-[14px_16px] sm:p-[14px_20px] ${i < trustRows.length - 1 ? "border-b border-border" : ""}`}><span aria-hidden="true" className="flex size-[42px] shrink-0 items-center justify-center rounded-full bg-accent-soft text-accent"><row.icon className="size-5" /></span><div className="min-w-0"><strong className="block break-words text-sm font-bold text-foreground">{row.title}</strong><span className="break-words text-label text-muted">{row.sub}</span></div></div>)}</article>
