...
export default function ProductDetail() {
  ...
  return (
    <main className="mx-auto max-w-[1400px] overflow-x-hidden px-[clamp(12px,3vw,24px)] py-4 pb-16 sm:py-6">
      ...
      <button aria-label="Go back" className="mb-4 inline-flex min-h-11 items-center gap-1.5 rounded-full px-3 py-2 font-semibold text-foreground transition-colors hover:bg-surface-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent sm:mb-5" type="button" onClick={() => navigate(-1)}><ArrowLeft aria-hidden="true" className="size-4" /><span>Back</span></button>
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-2 lg:gap-10">
        ...
        <div className="flex min-w-0 flex-col lg:pt-2">
          ...
          <article className="mb-5 mt-4 overflow-hidden rounded-[22px] border border-border bg-surface-secondary shadow-[0_4px_16px_rgba(0,0,0,0.04)] sm:mt-6 sm:rounded-[24px]">{trustRows.map((row, i) => <div key={row.title} className={`flex items-center gap-3.5 p-[14px_16px] sm:p-[14px_20px] ${i < trustRows.length - 1 ? "border-b border-border" : ""}`}><span aria-hidden="true" className="flex size-[42px] shrink-0 items-center justify-center rounded-full bg-accent-soft text-accent"><row.icon className="size-5" /></span><div className="min-w-0"><strong className="block break-words text-sm font-bold text-foreground">{row.title}</strong><span className="break-words text-[13px] text-muted">{row.sub}</span></div></div>)}</article>
