...
import { Calendar, CheckCircle2, Clock, Key, LogOut, Mail, Package, Phone, RefreshCw, ShieldCheck, User, Wrench, AlertTriangle } from "lucide-react";
...
export default function Account() {
  ...
  const tabItems = [
    ...
    { id: "bookings" as const, label: `Bookings (${bookings.length})`, icon: Wrench },
    { id: "orders" as const, label: `Orders (${orders.length})`, icon: Package },
  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-5 sm:px-6 sm:py-8 lg:px-8">
      ...
      <Card className="mb-6 p-4 sm:mb-8 sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            <div aria-hidden="true" className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-accent-soft text-accent sm:size-14"><User className="size-6 sm:size-7" /></div>
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="truncate text-xl font-bold text-foreground sm:text-2xl">Hello, {displayName}</h1><Chip size="sm" variant="soft"><Chip.Label>Customer</Chip.Label></Chip></div><p className="truncate text-sm text-muted">{user.email}</p></div>
          </div>
          <Button className="min-h-11 w-full sm:w-auto" variant="outline" onPress={handleLogout}><LogOut aria-hidden="true" className="size-4" /><span>Sign Out</span></Button>
        </div>
      </Card>
      ...
      <Tabs className="w-full" selectedKey={activeTab} onSelectionChange={(key) => setActiveTab(String(key) as AccountTab)} variant="secondary">
        <Tabs.ListContainer className="mb-6">
          <Tabs.List aria-label="Account sections" className="min-w-max">
            {tabItems.map(({ id, label, icon: Icon }, index) => (
              <Tabs.Tab key={id} id={id} className="min-h-11 shrink-0 px-3 text-sm sm:px-5">
        ...
        <Tabs.Panel id="profile">
          <Card className="p-4 sm:p-8">
            <Card.Header className="pb-4"><Card.Title className="text-xl">Contact Information</Card.Title><Card.Description>Update your personal contact details for repair notifications and orders.</Card.Description></Card.Header>
            <Card.Content>
              ...
              <Form className="flex flex-col gap-5" onSubmit={handleUpdateProfile}>
                ...
                <Button className="min-h-11 w-full sm:w-auto sm:self-start" isDisabled={profileSaving} type="submit" variant="primary">{profileSaving ? <><Spinner size="sm" /><span>Saving...</span></> : <span>Save Changes</span>}</Button>
              </Form>
            </Card.Content>
          </Card>
        </Tabs.Panel>
        ...
        <Tabs.Panel id="bookings">
          <div className="flex flex-col gap-4"><div className="flex flex-col gap-3 min-[420px]:flex-row min-[420px]:items-center min-[420px]:justify-between"><h2 className="text-xl font-bold text-foreground">Your Repair Bookings</h2><Button className="min-h-11" size="sm" variant="outline" onPress={loadUserData}><RefreshCw aria-hidden="true" className="size-3.5" /><span>Refresh</span></Button></div>
            {dataLoading ? <LoadingCard /> : bookings.length === 0 ? <EmptyCard icon={<Wrench className="size-10" />} title="No bookings found" description="You haven’t submitted any repair appointments yet." action="Book a Repair" onAction={() => navigate("/repairs")} /> : <div className="grid grid-cols-1 gap-4">{bookings.map((booking) => <Card key={booking.id} className="p-4 sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><div aria-hidden="true" className="flex size-10 shrink-0 items-center justify-center rounded-full bg-accent-soft text-accent"><Wrench className="size-5" /></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-foreground">{booking.service}</h3><Chip color={booking.status === "completed" ? "success" : booking.status === "cancelled" ? "danger" : "warning"} size="sm" variant="soft"><Chip.Label className="capitalize">{booking.status || "Pending"}</Chip.Label></Chip></div><p className="text-sm font-medium text-foreground">{booking.device_type} {booking.device_model ? `— ${booking.device_model}` : ""}</p>{booking.issue && <p className="mt-1 text-xs text-muted">Issue: {booking.issue}</p>}</div></div><div className="flex flex-row gap-4 border-t border-border pt-3 text-xs text-muted sm:flex-col sm:gap-1 sm:border-t-0 sm:pt-0 sm:text-right"><span className="flex items-center gap-1.5"><Calendar aria-hidden="true" className="size-3.5" />{booking.appt_date}</span><span className="flex items-center gap-1.5"><Clock aria-hidden="true" className="size-3.5" />{booking.appt_time}</span></div></div></Card>)}</div>}
          </div>
        </Tabs.Panel>

        <Tabs.Panel id="orders">
          <div className="flex flex-col gap-4"><div className="flex flex-col gap-3 min-[420px]:flex-row min-[420px]:items-center min-[420px]:justify-between"><h2 className="text-xl font-bold text-foreground">Order History</h2><Button className="min-h-11" size="sm" variant="outline" onPress={loadUserData}><RefreshCw aria-hidden="true" className="size-3.5" /><span>Refresh</span></Button></div>
            {dataLoading ? <LoadingCard /> : orders.length === 0 ? <EmptyCard icon={<Package className="size-10" />} title="No orders yet" description="Your order history is currently empty." action="Browse Shop" onAction={() => navigate("/shop")} /> : <div className="grid grid-cols-1 gap-4">{orders.map((order) => <Card key={order.id} className="p-4 sm:p-5"><div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-border pb-3"><div><span className="text-xs font-semibold text-muted">Order ID</span><p className="font-mono text-sm font-bold text-foreground">#{order.id.slice(0, 8)}</p></div><div className="text-right"><Chip color={order.status === "delivered" ? "success" : order.status === "cancelled" || order.status === "refunded" ? "danger" : "warning"} size="sm" variant="soft"><Chip.Label className="capitalize">{order.status}</Chip.Label></Chip><p className="mt-1 text-xs text-muted">{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : ""}</p></div></div><div className="flex flex-col gap-2">{order.items?.map((item, index) => <div key={index} className="flex items-start justify-between gap-4 text-sm"><span className="min-w-0 text-foreground">{item.name} <span className="text-muted">× {item.qty}</span></span><span className="shrink-0 font-semibold text-foreground">${(item.price * item.qty).toFixed(2)}</span></div>)}</div><div className="mt-4 flex items-center justify-between border-t border-border pt-3 text-sm"><span className="font-bold text-foreground">Total</span><span className="text-base font-bold text-accent">${order.total?.toFixed(2) || "0.00"}</span></div></Card>)}</div>}
          </div>
        </Tabs.Panel>

        <Tabs.Panel id="settings">
          <Card className="p-4 sm:p-8"><Card.Header className="pb-4"><Card.Title className="text-xl">Change Password</Card.Title><Card.Description>Ensure your account is using a strong password.</Card.Description></Card.Header><Card.Content>
            ...
            <Form className="flex flex-col gap-5" onSubmit={handleChangePassword}>
              ...
              <Button className="min-h-11 w-full sm:w-auto sm:self-start" isDisabled={passwordSaving} type="submit" variant="primary">{passwordSaving ? <><Spinner size="sm" /><span>Updating Password...</span></> : <span>Update Password</span>}</Button>
            </Form>
          </Card.Content></Card>
        </Tabs.Panel>
      </Tabs>
...
function LoadingCard() { return <div className="flex min-h-[200px] items-center justify-center rounded-2xl border border-border bg-surface p-8"><Spinner /></div>; }
function EmptyCard({ icon, title, description, action, onAction }: { icon: React.ReactNode; title: string; description: string; action: string; onAction: () => void }) { return <Card className="p-6 text-center sm:p-8"><div aria-hidden="true" className="mx-auto mb-3 flex size-14 items-center justify-center rounded-full bg-accent-soft text-accent">{icon}</div><h3 className="text-lg font-semibold text-foreground">{title}</h3><p className="mt-1 text-sm text-muted">{description}</p><Button className="mt-4 min-h-11" variant="primary" onPress={onAction}>{action}</Button></Card>; }
