        ...
        <Modal.Container className="pb-24 lg:pb-0" scroll="inside" size="lg">
          <Modal.Dialog>
            {done ? (
              ...
              <>
                <Modal.Header><div><Modal.Heading>Home Visit</Modal.Heading><p className="m-0 text-sm text-muted">Is this a residential or commercial address?</p></div><Modal.CloseTrigger /></Modal.Header>
                <Modal.Body className="flex flex-col gap-4 py-6">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Modal.Footer><Button variant="outline" onPress={() => setVisitType("")}><ChevronLeft className="size-4" /><span>Back</span></Button></Modal.Footer>
              </>
            ) : (
              <>
                <Modal.Header><div><Modal.Heading>Book Appointment</Modal.Heading><div className="flex items-center gap-2"><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-caption font-bold ${visitType === "home" ? "bg-accent-soft text-accent" : "bg-surface-secondary text-muted"}`}>{visitType === "home" ? <><Home className="size-3" /><span>Home Visit — {form.visitLocationType === "commercial" ? "Commercial" : "Residential"}</span></> : <><Store className="size-3" /><span>In-Store</span></>}</span><p className="m-0 text-sm text-muted">Step {step + 1} of {STEPS.length} — {STEPS[step]}</p></div></div><Modal.CloseTrigger /></Modal.Header>
