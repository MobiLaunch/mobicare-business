    ...
    <header className="fixed inset-x-0 top-0 z-40 min-h-14 border-b border-border bg-surface/90 px-3 backdrop-blur-xl sm:px-4" id="site-header">
      <div className="mx-auto grid min-h-14 w-full max-w-[1400px] grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-4">
        <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
          <Button isIconOnly aria-label={`Switch to ${theme === "dark" ? "Light" : "Dark"} Mode`} className="size-9 shrink-0" variant="ghost" onPress={() => setColorScheme?.(theme === "dark" ? "light" : "dark")}>
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-0.5 rounded-full bg-surface-secondary p-0.5 lg:flex">
            {NAV_LINKS.map(({ to, label, icon: Icon }) => {
              ...
              return <Link key={to} aria-current={active ? "page" : undefined} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] font-medium transition-colors ${active ? "bg-accent text-accent-foreground" : "text-foreground hover:bg-surface"}`} href={to}>
                <Icon aria-hidden="true" className="size-3.5" />
                <span>{label}</span>
              </Link>;
            })}
            <button aria-label={`Open Shopping Cart${cartItemCount > 0 ? `, ${cartItemCount} items` : ""}`} className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] font-medium text-foreground transition-colors hover:bg-surface" title="Open Shopping Cart" type="button" onClick={() => setCartDrawerOpen(true)}>
        ...
        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
          ...
          <Link aria-label={user ? "My Account" : "Sign in"} className={`hidden items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] font-medium lg:inline-flex ${accountActive ? "bg-surface-secondary text-foreground" : "text-foreground hover:bg-surface-secondary"}`} href={accountPath}>
