...
export function getClient() {
  ...
  _client = createClient(url, anonKey, {
    ...
    // NO custom headers — authentication is handled via Supabase Auth JWTs
    // and Row Level Security policies, not passcode headers
  })
  _clientUrl = url
  _clientKey = anonKey
  return _client
...
// ─── Supabase Auth ─────────────────────────────────────────────────────────
// All admin authentication goes through Supabase Auth.
// The admin user must be created in Supabase Dashboard → Authentication → Users.
