...
function ticketNumber(id: string) {
  return `MC-${id.replace(/[^a-zA-Z0-9]/g, "").slice(-8).toUpperCase() || "BOOKING"}`;
}

function Ticket({ booking, service }: { booking: BookingRecord; service: string }) {
  const home = booking.visit_type === "home" || booking.visitType === "home";
  return (
    <div className="relative overflow-hidden rounded-[24px] border border-border bg-surface shadow-sm">
      <div className="flex items-start justify-between gap-4 p-5">
        <div>
          <p className="m-0 text-[10px] font-bold uppercase tracking-[0.18em] text-muted">Mobicare Service Ticket</p>
          <p className="m-0 mt-1 font-mono text-lg font-extrabold tracking-tight text-foreground">#{ticketNumber(booking.id)}</p>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${STATUS_STYLES[booking.status] || "bg-surface-tertiary text-foreground"}`}>{cap(booking.status || "pending")}</span>
      </div>
      <div className="mx-5 border-t border-dashed border-border" />
      <div className="grid grid-cols-2 gap-4 p-5 text-sm sm:grid-cols-4">
        <div><span className="block text-[10px] font-bold uppercase tracking-wide text-muted">Appointment</span><strong className="mt-1 block text-foreground">{booking.appt_date}</strong><span className="text-xs text-muted">{booking.appt_time}</span></div>
        <div><span className="block text-[10px] font-bold uppercase tracking-wide text-muted">Customer</span><strong className="mt-1 block truncate text-foreground">{booking.customer_name}</strong><span className="text-xs text-muted">{booking.customer_phone || booking.customer_email || "No contact"}</span></div>
        <div><span className="block text-[10px] font-bold uppercase tracking-wide text-muted">Service</span><strong className="mt-1 block truncate text-foreground">{service}</strong><span className="text-xs text-muted">{booking.device_type}</span></div>
        <div><span className="block text-[10px] font-bold uppercase tracking-wide text-muted">Visit</span><strong className="mt-1 flex items-center gap-1.5 text-foreground">{home ? <Home className="size-3.5 text-accent" /> : <Store className="size-3.5 text-accent" />}{home ? "Home Visit" : "In-Store"}</strong><span className="text-xs text-muted">{booking.device_model}</span></div>
      </div>
      ...
      <div className="flex items-center justify-between border-t border-dashed border-border bg-surface-secondary/30 px-5 py-3">
        <span className="font-mono text-[9px] tracking-[0.28em] text-muted">||||| |||| ||||| ||</span>
        <span className="text-[10px] text-muted">Created {booking.created_at ? new Date(booking.created_at).toLocaleDateString() : "—"}</span>
      </div>
    </div>
  );
}
...
export default function Bookings() {
  ...
  const load = async () => {
    ...
    setLoading(false);
    if (data) setBookings(data); else addToast("Could not load appointments from Supabase", "error");
  };
  ...
  const columns: AdminDataTableColumn<BookingRecord>[] = [
    ...
    { key: "service", header: "Service", render: (b) => <div className="flex items-center gap-1.5"><Wrench className="size-4 text-accent" /><strong className="text-[13px] capitalize">{serviceLabel(b)}</strong></div> },
    { key: "device", header: "Device", render: (b) => <div><strong className="block text-[13px] text-foreground">{b.device_type}</strong><span className="text-xs text-muted">{b.device_model}</span></div> },
    { key: "visit", header: "Visit", render: (b) => <span className="flex items-center gap-1.5 text-xs font-semibold">{b.visit_type === "home" || b.visitType === "home" ? <><Home className="size-3.5 text-accent" />Home</> : <><Store className="size-3.5 text-accent" />Store</>}</span> },
